"""Audit the top-500 non-signal HerbPairIAM predictions against PMDA inserts.

Produces ``top500_nonsignal_pmda_audit.csv`` in the same 9-column
format as ``top200_nonsignal_pmda_audit.csv`` so downstream
``render_external_validation_tex.py`` can pick up the new file and
regenerate Main Table 5 directly. Adds per-ADR-family aggregate so
the new table caption can cite per-family numbers.
"""
...
OUT_CSV = OOF_PKL.parent / "top500_nonsignal_pmda_audit.csv"
...
# ============================================================
def _hep():
    return {"direct": ["肝機能障害", "肝機能異常", "肝障害", "肝炎", "黄疸",
                       "劇症肝炎", "肝不全", "胆汁うっ滞", "胆石", "胆管炎",
                       "腹水", "AST", "ALT", "ALP", "γ-GTP", "γGTP", "Al-P",
                       "アルカリホスファターゼ", "ビリルビン", "トランスアミナーゼ"], "symptom": []}
...
def _match_adr(name: str):
    ...
    if any(k in n for k in ("HEPAT", "LIVER", "JAUNDICE", "CHOLESTASIS", "CHOLANGITIS",
                              "AMINOTRANSFERASE INCREASED", "ASCITES")): return "HEP", _hep()
    if any(k in n for k in ("INTERSTITIAL LUNG", "INTERSTITIAL PNEUMONITIS",
                              ...
                              "PULMONARY ALVEOLAR HAEMORRHAGE")): return "ILD", _ild()
    if any(k in n for k in ("HYPOKAL", "PSEUDOALDOSTERONISM", "RHABDOMYOLYSIS", "MYOPATHY")): return "HPK", _hpk()
    if any(k in n for k in ("RASH", "ERYTHEMA", "URTICARIA", "ERUPTION", "TOXIC SKIN",
                              "STEVENS", "EPIDERMAL", "DERMATITIS", "PRURITUS",
                              "SKIN REACTION", "ECZEMA", "PUSTUL")): return "SKN", _skn()
    if any(k in n for k in ("APPETITE", "NAUSEA", "VOMIT", "DIARRHOEA", "DIARRHEA",
                              "ABDOMINAL PAIN", "CONSTIPAT", "ILEUS")): return "DIG", _dig()
    if any(k in n for k in ("HEADACHE", "DIZZINESS", "INSOMNIA", "SOMNOLENCE", "FATIGUE",
                              ...
                              "PARALYSIS", "GAIT DISTURBANCE", "ABNORMAL BEHAV", "SCHIZOPH")): return "NS", _ns()
    if any(k in n for k in ("PALPITATIONS", "TACHYCARDIA", "ARRHYTHMIA", "HYPERTENS",
                              "HYPOTENS", "OEDEMA", "EDEMA", "MYOCARDIAL INFARCTION",
                              "HEART FAILURE")): return "CARD", _card()
    if any(k in n for k in ("HYPONATRAEMIA", "HYPONATREMIA", "HYPOCALCAEMIA", "HYPOCALCEMIA",
                              "HYPERCALCAEMIA", "HYPERCALCEMIA", "ELECTROLYTE")): return "ELE", _ele()
    if any(k in n for k in ("PLATELET", "THROMBOCYTOPENIA", "ANAEMIA", "ANEMIA",
                              "LEUKOCYTO", "LEUCOCYTO", "NEUTROPENIA", "PANCYTOPENIA")): return "BLD", _bld()
    if any(k in n for k in ("URINARY RETENTION", "RENAL FAILURE", "NEPHRITIS", "KIDNEY")): return "URO", _uro()
...
def _scan(text: str, syn) -> tuple[bool, str]:
    if syn is None:
        return False, ""
...
def main() -> int:
    ...
    print(f"[top500] built non-signal top-{TOP_N}; "
          f"score range [{non_signal['prob'].min():.3f}, {non_signal['prob'].max():.3f}]")
    ...
    print(f"[top500] wrote {OUT_CSV}  ({len(out)} rows)")
    print(f"[top500] Yes = {n_yes} / {len(out)}  ({100*n_yes/len(out):.1f}%)")
    print()
    print("ADR family breakdown:")
    for fam, sub in out.groupby("adr_family"):
        y = int((sub["pmda_support"] == "Yes").sum())
        n = len(sub)
        print(f"  {fam:<10}  Yes={y:3d}/{n:4d}  rate={100*y/n:5.1f}%")
    return 0
