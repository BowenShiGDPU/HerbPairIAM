"""Train HerbPairIAM with the frozen canonical configuration.

Writes ``HerbPairIAM_fold*.pkl`` (predictions + metrics) and
``HerbPairIAM_fold*.pt`` (model state dicts) under
``$RESULTS_ROOT_DIR/$EXPERIMENT_SUBDIR``. Folds whose ``.pkl`` already
exists are skipped unless ``--no-skip`` is passed.
...
from scripts.prepare_inputs import PRIMARY_MODEL_NAME, prepare_common_inputs
...
def frozen_cfg() -> ModelConfig:
    """Frozen canonical config for HerbPairIAM."""
    return ModelConfig(
        hidden=32,
        dropout=0.3,
        lr=1e-3,
